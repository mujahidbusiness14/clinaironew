...
## If notification emails stop arriving

The EmailJS call now logs its result. Open the browser console, submit a form, and look
for one of these:

- `[Clinairo] notification email sent 200 OK` — working
- `[Clinairo] EmailJS FAILED — lead IS saved in Firestore. status: 4xx text: ...`

The lead is always written to Firestore first, so a failed email never loses an enquiry.
Common status codes:

| Status | Meaning |
|---|---|
| 400 | Template variable mismatch — check the variable names in your EmailJS template |
| 403 | Public key rejected, or the API is restricted to specific origins in your account settings |
| 412 | Gmail/Outlook service needs re-authorising in the EmailJS dashboard |
| 426 | Monthly quota exhausted on the free plan (200 emails) |

The template receives many aliases (`name`, `from_name`, `user_name`, `email`,
`from_email`, `reply_to`, `phone`, `clinic_name`, `message`, and others) so it renders
whichever variable names your template was built with.
